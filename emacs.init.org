* Description
                ...
                (add-to-list 'load-path "~/git/dotemacs") ;;where emacs.init.el is
                (add-to-list 'load-path "~/.emacs.d") ;;where packages are
                (load "emacs.init")
                #+END_SRC

                By doing this, Emacs will look for files on =~/git/dotemacs=; there,
                he will find =emacs.init.el= (the file produced by tangling
                =emacs.init.org=) and will load it.

                I also use another trick. In my =.emacs=, I force my =~/git/dotemacs=
                to pull modifications from =github= before loading Emacs. By doing
                that, everything will always be updated on all my computers.
...
A very useful package is git-timemachine (installed from MELPA). It
has a very simple usage. To start, M-x git-timemachine (binding to
'tm'); to navigate, 'n' and 'p'; to exit, 'q'.
...
Since July 25th 2014, I am using the version from MELPA.
...
From http://www.emacswiki.org/emacs/MakefileMode
